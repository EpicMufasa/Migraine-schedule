...
for i, date in enumerate(dates):
    with status_cols[i]:
        # Use consistent date string key
        date_key = date.strftime('%Y-%m-%d')
        # Gather shifts, skip missing entries
        day_shifts = []
        for name in st.session_state.pharmacists:
            rec = st.session_state.schedule.get(name, {}).get(date_key)
            if rec and 'start' in rec and 'end' in rec:
                day_shifts.append(rec)
        # Validate
        valid = validate_day(day_shifts)
        color = "green" if valid else "red"
        label = 'OK' if valid else 'MISSING'
        st.markdown(
            f"<div style='background-color:{color};padding:10px;margin-bottom:4px;border-radius:5px;text-align:center;'>"
            f"{date.strftime('%b %d')}<br>{label}</div>",
            unsafe_allow_html=True
        )

# 8. Download schedule Download schedule
